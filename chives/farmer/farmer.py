...
from chives.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from chives.util.config import load_config, lock_and_load_config, save_config, config_path_for_filename
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "xcc_target_address" not in self.config:
            self.config = config["farmer"]
        if "xcc_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xcc_target_address" not in self.config or "xcc_target_address" not in self.pool_config:
            log.debug("xcc_target_address missing in the config")
            return False
        ...
        self.pool_target_encoded = self.pool_config["xcc_target_address"]
