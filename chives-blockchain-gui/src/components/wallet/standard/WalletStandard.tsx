...
import { /* mojo_to_chives_string, */ chives_to_mojo } from '../../../util/chives';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_chives_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of chives in the blockchain at the current
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      openDialog(
        <AlertDialog>
          <Trans>
            Error: Cannot send chives to coloured address. Please enter a chives
            address.
          </Trans>
        </AlertDialog>,
      );
    ...
    if (address.slice(0, 12) === 'chives_addr://') {
    ...
    const amountValue = Number.parseFloat(chives_to_mojo(amount));
    const feeValue = Number.parseFloat(chives_to_mojo(fee));
