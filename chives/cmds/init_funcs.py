...
from chives.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_chives_ca_crt_key,
    make_ca_cert,
...
from chives.util.ssl_check import (
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xcc_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xcc_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "xcc_target_address" not in config["farmer"]:
        print(
            f"Setting the xcc destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["xcc_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["xcc_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xcc_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xcc_target_address" not in config["pool"]:
        print(f"Setting the xcc destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["xcc_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["xcc_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xcc_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the XCC destination addresses, edit the `xcc_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    write_ssl_cert_and_key(chives_ca_crt_path, chives_ca_crt, chives_ca_key_path, chives_ca_key)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                chives_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return chives_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)
