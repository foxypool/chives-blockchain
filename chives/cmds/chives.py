...
from chives.cmds.passphrase import passphrase_cmd
...
from chives.cmds.plotnft import plotnft_cmd
from chives.cmds.plotters import plotters_cmd
from chives.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from chives.util.ssl_check import check_ssl
...
def cli(
    ...
    if passphrase_file is not None:
        from chives.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from chives.cmds.passphrase_funcs import remove_passphrase_options_from_cmd
...
@click.option(
    ...
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by chives start <service>
)
@click.pass_context
