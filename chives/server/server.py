...
from chives.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
...
class ChivesServer:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use chives's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.chivescoin.org/") as resp:
                    if resp.status == 200:
