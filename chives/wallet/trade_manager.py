...
from chives.wallet.cc_wallet import cc_utils
from chives.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from chives.wallet.cc_wallet.cc_wallet import CCWallet
from chives.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
...
from chives.wallet.util.trade_utils import (
...
class TradeManager:
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_chives(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        if chives_discrepancy is not None:
            chives_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_chives(
                chives_discrepancy, []
            )
            if chives_spend_bundle is not None:
                for coinsol in coinsols:
                    chives_spend_bundle.coin_spends.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if chives_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = chives_spend_bundle.removals()
        ...
        if chives_spend_bundle is not None:
            ...
            if chives_discrepancy < 0:
                tx_record = TransactionRecord(
                    ...
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=chives_spend_bundle,
                    additions=chives_spend_bundle.additions(),
                    removals=chives_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=chives_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
