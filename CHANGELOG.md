...
- Beta builds are built on every merge to main, and are now available from <https://chivescoin.org/download/>.
...
- Switched IP address lookup to first use Chives's service ip.chivescoin.org.
...
- Mainnet uses port 9699 and other constants and service names were changed for mainnet.
...
- This is a hard fork/breaking change from RC6/7. TXCC Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chivescoin.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `chives farm summary` will now display TXCC or XCC as appropriate.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.chivescoin.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.chivescoin.org via https, for example.
...
- RC3 is a new chain to support the last major chiveslisp changes. TXCC from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'chives wallet send' now takes in TXCC or XCC as units instead of mojos.
...
- The cli wallet, `chives wallet`, was incorrectly displaying TXCC balances as if they were Coloured Coins.
...
- Testnet is now running at the primary port of 59699. Update your routers appropriately. This opens 9699 for mainnet.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chives Network at backup.chivescoin.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chives backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chivescoin.org, your own installation, or a third party's version of it.
...
other chains.
- Keys are now derived with the EIP 2334 (m/12381/9699/a/b).
...
- The user interface now refers to chives coins with their correct currency code of XCC.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 9699 or their UPnP port.
...
- More details on the release at [https://www.chivescoin.org/developer/](https://www.chivescoin.org/developer/)
