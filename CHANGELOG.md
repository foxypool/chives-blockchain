...
Farmers rejoice: today's release integrates two plotters in broad use in the Chives community: Bladebit, created by @harold-b, and Madmax, created by @madMAx43v3r. Both of these plotters bring significant improvements in plotting time. More plotting info [here](https://github.com/Chives-Network/chives-blockchain/wiki/Alternative--Plotters).
...
- Performance improvements for nodes to support higher transaction volumes, especially for low powered devices like RaspBerry Pi. Full details at [#9050](https://github.com/Chives-Network/chives-blockchain/pull/9050).
...
- Integrated the Bladebit plotter to CLI and GUI. Thanks @harold-b for all your hard work on this, and welcome again to the Chives Network team!
- Added the Madmax plotter to CLI and GUI. Thanks @madMAx43v3r for your support!
- Added option to configure your node to testnet using to `chives init --testnet`.
...
- PlotNFT transactions via CLI (e.g. `chives plotnft join`) now accept a fee parameter, but it is not yet operable.

## 1.2.10 Chives blockchain 2021-10-25

We have some great improvements in this release: We launched our migration of keys to a common encrypted keyring.yaml file, and we secure this with an optional passphrase in both GUI and CLI. We've added a passphrase hint in case you forget your passphrase. More info on our [wiki](https://github.com/Chives-Network/chives-blockchain/wiki/Passphrase-Protected-Chives-Keys-and-Key-Storage-Migration). We also launched a new Chiveslisp compiler in clvm_tools_rs which substantially improves compile time for Chiveslisp developers. We also addressed a widely reported issue in which a system failure, such as a power outage, would require some farmers to sync their full node from zero. This release also includes several other improvements and fixes.

### Added

- Added support for keyring migration from keychain, and the addition of passphrase support. Learn more at our [wiki](https://github.com/Chives-Network/chives-blockchain/wiki/Passphrase-Protected-Chives-Keys-and-Key-Storage-Migration).
- Enabled experimental use of a new Chiveslisp compiler in clvm_tools_rs in chives-blockchain, which is off by default, and substantially improves compile time.
...
- Thanks @cross for change that allows using IPv6 address in config.yaml for remote harvesters and other chives services.
...
- Updated the old comment on chives/util/streamable.py with newer developer documentation.
...
- Thanks @jcteng for fixing a bug on the Chives DID wallet that showed 'mojo:'' instead of 'mojo'.

## 1.2.9 Chives blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Chives blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Chives's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://chives.net/download/>.
...
- We have made many full node changes to support our upcoming Chives Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Chives's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Chives Asset Token (CAT) standard.
...
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing Chiveslisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
- Thanks @altendky for changing the default to paginate to chives wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Chives's service ip.chives.net.
...
## 1.2.3 Chives blockchain 2021-07-26
...
- Included all Chiveslisp files in source distribution.
...
- Thanks @mishan for your change to 'chives plotnft show' to display Percent Successful Points.
...
## 1.2.2 Chives blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Chives-Network/chives-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Chives blockchain 2021-07-12
...
## 1.2.0 Chives blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `chives plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Chives-Network/chives-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Chives-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `chives farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `chives configure` now supports command line updates to peer count and target peer count.
...
- All Chives repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `chives plotnft` to manage pools.
...
- We have increased the strictness of validating Chiveslisp in the mempool and clvm.
...
- Correctly display private key in `chives keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `chives keys show`.
...
- Thank you to @ChivesMineJP for various improvements.
...
- `chives farm summary` better handles wallet errors.
...
### 1.1.7 Chives Blockchain 2021-06-05
...
- We added a simple profiler to track performance times in the application (see /chives/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `chives configure -t true|false`).
...
- Secret wallet key is hidden by default in `chives keys show`. Use `chives keys show --show-mnemonic-seed` for private keys.
...
- We changed chiveslisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Chiveslisp programs now remained serialized in Node for better performance.
...
- This is the 1.0 release of Chiveslisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [chiveslisp.com](https://chiveslisp.com) shortly.
...
- You should review the [release notes for v1.0.2](https://github.com/Chives-Network/chives-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `chives keys`.
...
- We have released version 1.0.0 of [chiapos](https://github.com/Chives-Network/chiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/chives-blockchain).
...
- @jespino changed `chives keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `chives keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `chives keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/chives-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/chives-blockchain).
...
- Migration with CHIVES_ROOT set does not crash chives init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chives.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `chives farm summary` will now display TXCH or XCH as appropriate.
...
- `chives farm summary` displayed some incorrect amounts.
...
- `chives netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `chives netspace`. Thanks @altendky.
- There was a typo in `chives plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Chives-Network/chives-blockchain/issues/1189)
...
- Remove `chives plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
...
- We now hand build Apple Silicon native binary wheels for all chives-blockchain dependencies and host them at [https://pypi.chives.net/simple](https://pypi.chives.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a chiveslisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during ChivesLisp native deserialization.
...
- We have moved to a new and much higher scalability download.chives.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.chives.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.chives.net/latest/Setup-MacOS.dmg).
- Chives wheels not on Pypi and some dependecies not found there also are now on pypi.chives.net.
...
- `chives farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `chives wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.chives.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `chives farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/chives-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `chives wallet` now takes get_address to get a new wallet receive address from the CLI.
- `chives plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- Chiveslisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Comments to Chiveslisp and clvm source have been updated for all of the Chiveslisp changes over the proceeding three weeks.
...
- Adding Click changed the behavior of `chives keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.chives.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the chives sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
...
- Various typos and style clean ups were made to the Click CLI implementation. `chives -upnp f` was added to disable uPnP.
- `chives plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major chiveslisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom chiveslisp programs. You need to make a SpendBundle, which includes the puzzle reveal (chiveslisp), a solution (chiveslisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/chives-blockchain).
- You can obtain a new wallet receive address on the command line with `chives wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All chiveslisp opcodes have been renumbered. This should be the last major breaking change for chiveslisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Chives-Network/chives-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `chives -h` and `chives SUBCOMMAND -h` can be your guide.
...
- `chives plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `chives netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `chives init` only migrates release candidate directories. The versioned sub directories under `~/chives` will be going away before mainnet.
...
- `chives configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Chives-Network/chives-blockchain/releases/tag/1.0rc1) was well.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Chives Blockchain GUI](https://crowdin.com/project/chives-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Chives-Network/chives-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Chives-Network/chives-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `chives plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'chives wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- You can now send and receive transactions with the command line. Try `chives wallet -h` to learn more. Also, `chives wallet` now requires a third argument of `show`, therefor you will use `chives wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [chives blockchain gui](https://crowdin.com/project/chives-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `chives init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `chives init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
...
- The GUI is now in a separate submodule repository from chives-blockchain, [chives-blockchain-gui](https://github.com/Chives-Network/chives-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `chives-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `chives wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Chives-Network/chives-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Chiveslisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Chives-Network/chives-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Chives-Network/chives-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `chives plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and ChivesLisp code matches the CI compiled code.
- We have removed the '-r' flag from `chives` as it was being overridden in most cases by the `-r` for restart flag to `chives start`. Use `chives --root-path` instead.
- `chives -h` now recommends `chives netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
...
- `chives show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `chives show` handles sub block 0 better.
...
- `chives plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `chives plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `chives show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `chives configure --enable-upnp`.
...
- `chives netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `chives show` of the GUI use `chives netspace -d 1000` as `chives netspace` defaults to `-d 192` which is one hour.
...
- There is a new cli command - `chives configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `chives configure -h`. Note that if chives services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- All appropriate Chiveslisp smart transactions have been updated to use aggsig_me.
...
- sha256tree has been removed from Chiveslisp.
- `chives show -s` has been refactored to support the new consensus.
- `chives netspace` has been refactored for new consensus.
...
- `chives show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Chives-Network/chives-blockchain/issues/500).
...
- In the GUI there was [a regression](https://github.com/Chives-Network/chives-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Chives-Network/chives-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Chives-Network/chives-blockchain/tree/dev/electron-react/src/locales) of the chives-blockchain repository.
...
- Updated to chiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [chiapos release notes](https://github.com/Chives-Network/chiapos/releases/tag/0.12.32).
...
- The rpc interfaces of all chives services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test chives since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve chives-blockchivesn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Chives Network at backup.chives.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Chives backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.chives.net, your own installation, or a third party's version of it.
...
- `chives init` now automatically discovers previous releases in each new release.

### Fixed

- `chives show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `chives show -w` with thanks to @pyl on Keybase.
...
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (chives run farmer-only, wallet-only, timelord-only)
...
- `chives show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `chives show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Chives)
...
- See wallet balances in command line: `chives show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `chives keys sign` and `chives keys verify` to allow farmers to certify their ownership of keys.
...
- `chives start wallet-server` changed to `chives start wallet`, for consistency.
...
- Removed legacy scripts such as chives-stop-server, chives-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Chives-Network/bls-signatures) to use the
...
"chives plots create" and "chives plots check".
- Add plot directories to config.yaml from the cli with "chives plots add".
...
- We added additional improvements and more RPCs to the start daemon and various services to continue to make chives start/stop reliable cross platform.
...
- Starting chives now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'chives keys' command line.
...
- We added a `chives keys` command line program, to see, add, and remove private keys.
...
- `chives netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `chives-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `chives-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `chives-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `chives-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `chives init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `chives start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `chives-start-harvester`.
...
- The user interface now refers to chives coins with their correct currency code of XCH.
...
- There was a regression in Chives Proof of Space ([chiapos](https://github.com/Chives-Network/chiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `chives init` then sync on first start will happen automatically. "\$CHIVES_ROOT" users will need to delete `$CHIVES_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `chives netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during chives init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `chives show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `chives show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Chives-Network/chives-blockchain/wiki) and placed in [INSTALL](https://github.com/Chives-Network/chives-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Chives-Network/chives-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of chives action commands. You can run `chives version` or `chives start node` for example. Just running `chives` will show you more options. However `chives-create-plots` continues to use the hyphenated form. Also it's now `chives generate keys` as another example.
- Chives start commands like `chives start farmer` and `chives stop node` now keep track of process IDs in a run/ directory in your configuration directory. `chives stop` is unlikely to work on Windows native for now. If `chives start -r node` doesn't work you can force the run/ directory to be reset with `chives start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Chives-Network/chives-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `chives start wallet-gui` has been chaned to `chives start wallet` which launches but the GUI and server on MacOS and Linux. `chives start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Chives-Network/chives-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `chives stop all` command sometimes fails, use `chives-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `chives init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `chives-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Chives-Network/chives-blockchain/wiki/Updating-beta-software). If you've set `$CHIVES_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `chives show`.

### Changed

- `chives-restart-harvester` has been renamed from `chives-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- There is now full transaction support on the Chives blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Chives-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the chives management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `chives-start-farmer &`. The [README.md](https://github.com/Chives-Network/chives-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- The libraries chives-pos, chives-fast-vdf, and chives-bip-158 have been moved to their own repositories: [chiapos](https://github.com/Chives-Network/chiapos), [chiavdf](https://github.com/Chives-Network/chiavdf), and [chaibip158](https://github.com/Chives-Network/chiabip158). They are brought in by chives-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Chives-Network/bls-signatures).
...
- Added color logging
- Now chives_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to chives_full node, chives_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new chives services were started.
...
- More details on the release at [https://www.chives.net/developer/](https://www.chives.net/developer/)

[unreleased]: https://github.com/Chives-Network/chives-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Chives-Network/chives-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Chives-Network/chives-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Chives-Network/chives-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Chives-Network/chives-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Chives-Network/chives-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Chives-Network/chives-blockchain/releases/tag/Alpha-1.0
