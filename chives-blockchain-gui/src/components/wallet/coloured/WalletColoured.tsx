...
import { AlertDialog, Card, Flex } from '@chives/core';
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_chives = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_chives = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_chives = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
    balancebox_3 +
    balance_ptotal_chives +
    balancebox_unit +
    balancebox_hline +
    balancebox_4 +
    balancebox_row +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('chives_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
